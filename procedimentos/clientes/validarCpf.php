...
class validarCpf
{
    ...
    public function ehValido($cpf){
        if(!validarCpf::ehCpf($cpf)) return false;

        $cpf_numeros = validarCpf::removeFormatacao($cpf);

        if(!validarCpf::verificarNumerosIguais($cpf_numeros)) return false;

        if(!validarCpf::validarDigitos($cpf_numeros)) return false;
